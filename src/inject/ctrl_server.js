...
class CtrlServer {
  constructor() {
    ...
    this.open(false)
    setInterval(() => this.sync_contacts(), 10000)
  ...
  static contactFactoryAddContact(real) {
    return (e) => {
      ...
      if (e.ContactFlag & confFactory.CONTACTFLAG_CONTACT || ! (e.UserName in _strangerContacts)) {
        window.ctrlServer.contacts.set(e.UserName, e)
        window.ctrlServer.pending_contacts.add(e.UserName)
      }
      return ret
    }
  }
  static contactFactoryDeleteContact(real) {
    return (e) => {
      window.ctrlServer.contacts.delete(e.UserName)
      window.ctrlServer.pending_contacts.add(e.UserName)
